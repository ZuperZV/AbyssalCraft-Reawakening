...
public class ModNoiseRouter {
    ...
    private static ResourceKey<DensityFunction> createKey(
            String name
    ) {
    ...
    private static ResourceKey<DensityFunction> createKeyWithDefaultNamespace(
            String name
    ) {
        return ResourceKey.create(
                Registries.DENSITY_FUNCTION,
    ...
    protected static NoiseRouter AbyssalWasteland(
            HolderGetter<DensityFunction> functions,
            HolderGetter<NormalNoise.NoiseParameters> noises,
            boolean largeBiomes,
            boolean amplified
        ...
        int veinMinY =
                Stream.of(
                                ModOreVeinifier.VeinType.values()
                        )
                        .mapToInt(
                                type -> type.minY
                        )
    ...
    private static DensityFunction offsetToDepth(
            DensityFunction offset
    ) {
    ...
    private static DensityFunction underground(
        ...
        DensityFunction spaghetti2DFunction =
                getFunction(
                        functions,
                        SPAGHETTI_2D
                );
        ...
        DensityFunction layerNoiseSource =
                DensityFunctions.noise(
                        noises.getOrThrow(
                                Noises.CAVE_LAYER
                        ),
        ...
        DensityFunction layerizedCavernsFunction =
                DensityFunctions.mul(
                        DensityFunctions.constant(
                                4.0F
                        ),
        ...
        DensityFunction solidifiedCheeseWithTopSlide =
                DensityFunctions.add(
                        DensityFunctions.add(
                                DensityFunctions.constant(
                                        0.27
        ...
        DensityFunction baseCaveDensity =
                DensityFunctions.add(
                        layerizedCavernsFunction,
                        solidifiedCheeseWithTopSlide
                );

        DensityFunction undergroundSubtractions =
                DensityFunctions.min(
                        DensityFunctions.min(
                                baseCaveDensity,
        ...
        DensityFunction pillarsWithoutCutoff =
                getFunction(
                        functions,
                        PILLARS
                );
        ...
        return DensityFunctions.max(
                undergroundSubtractions,
                pillars
        );
    }

    private static DensityFunction postProcess(
            DensityFunction slide
    ) {
        DensityFunction blended =
    ...
    private static DensityFunction slideOverworld(
            boolean isAmplified,
            DensityFunction caves
    ) {
    ...
    private static DensityFunction noiseGradientDensity(
        ...
        DensityFunction gradientUnscaled =
                DensityFunctions.mul(
                        depthWithJaggedness,
                        factor
                );
        ...
        return DensityFunctions.mul(
                ...
                gradientUnscaled.quarterNegative()
        );
    }
    ...
    private static DensityFunction preliminarySurfaceLevel(
        ...
        DensityFunction cachedFactor =
                DensityFunctions.cache2d(
                        factor
                );

        DensityFunction cachedOffset =
                DensityFunctions.cache2d(
                        offset
                );
        ...
        return DensityFunctions.findTopSurface(
                density,
                upperBound,
                -80,

                ModDimensions.ABYSSAL_WASTELAND_SETTINGS
                        .getCellHeight()
        );
    }
